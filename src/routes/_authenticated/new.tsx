...
export const Route = createFileRoute("/_authenticated/new")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Build a quick survey to share with your friends in under two minutes." },
    ],
  }),
  component: NewSurvey,
});
...
function NewSurvey() {
  ...
  function addQ(type: QType = "rating") { setQuestions((qs) => [...qs, { type, prompt: "", options: "" }]); }
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <Card className="space-y-4 p-5">
          <div className="space-y-1.5">
            ...
            <Input id="title" placeholder="e.g. Which deodorant should I switch to?"
              value={title} onChange={(e) => setTitle(e.target.value)} maxLength={120} required />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="description">Description (optional)</Label>
            <Textarea id="description" placeholder="A little context for your friends…"
              value={description} onChange={(e) => setDescription(e.target.value)} maxLength={600} rows={3} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="category">Category (optional)</Label>
            <Input id="category" placeholder="Books, movies, personal care…"
              ...
              <li key={i}>
                <Card className="space-y-3 p-4">
                    ...
                    <div className="ml-auto flex items-center gap-2">
                      <Select value={q.type} onValueChange={(v) => updateQ(i, { type: v as QType })}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="product_suggestion">Suggest a product</SelectItem>
                        </SelectContent>
                      </Select>
                      <Button type="button" variant="ghost" size="icon"
                        aria-label={`Remove question ${i + 1}`}
                        onClick={() => removeQ(i)} disabled={questions.length === 1}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input id={`q-${i}-prompt`}
                      placeholder={q.type === "product_suggestion" ? "e.g. What deodorant do you use?" : "What do you think about…"}
                      value={q.prompt} onChange={(e) => updateQ(i, { prompt: e.target.value })} maxLength={280} />
                  </div>
                  {q.type === "choice" && (
                    <div className="space-y-1.5">
                      <Label htmlFor={`q-${i}-options`}>Options (one per line)</Label>
                      <Textarea id={`q-${i}-options`} placeholder={"Option A\nOption B\nOption C"}
                        value={q.options} onChange={(e) => updateQ(i, { options: e.target.value })} rows={4} />
                    </div>
                  )}
                  {q.type === "product_suggestion" && (
                    <p className="text-xs text-muted-foreground">
                      Respondents type a product name; we'll suggest matching links (Amazon, Etsy, creators) and let them pick one.
                    </p>
                  )}
                </Card>
              </li>
        ...
        <section aria-labelledby="aff-heading" className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Paste product links (Amazon, Etsy, creator sites). They'll appear on the results page with a clear affiliate disclosure.
          </p>
          <ul className="space-y-3">
            {affiliates.map((a, i) => (
              <li key={i}>
                <Card className="space-y-2 p-3">
                  <div className="flex gap-2">
                    <Input aria-label={`Product ${i + 1} label`} placeholder="Label"
                      value={a.label}
                      onChange={(e) => setAffiliates((xs) => xs.map((x, idx) => (idx === i ? { ...x, label: e.target.value } : x)))} />
                    <Button type="button" variant="ghost" size="icon"
                      aria-label={`Remove product link ${i + 1}`}
                      onClick={() => setAffiliates((xs) => xs.filter((_, idx) => idx !== i))}>
                      <Trash2 className="h-4 w-4" aria-hidden="true" />
                    </Button>
                  </div>
                  <div className="flex gap-2">
                    <Input aria-label={`Product ${i + 1} URL`} placeholder="https://…"
                      value={a.url}
                      onChange={(e) => setAffiliates((xs) => xs.map((x, idx) => (idx === i ? { ...x, url: e.target.value } : x)))} />
                    <Select value={a.source} onValueChange={(v) => setAffiliates((xs) => xs.map((x, idx) => (idx === i ? { ...x, source: v as AffDraft["source"] } : x)))}>
          ...
          <Button type="button" variant="outline" className="w-full rounded-full"
            onClick={() => setAffiliates((xs) => [...xs, { label: "", url: "", source: "other" }])}>
